...
struct PrintHomeView: View {
    ...
    var body: some View { NavigationStack { ScrollView { VStack(alignment: .leading, spacing: 20) {
        ...
        Button { store.makePDF() } label: { Label("印刷用PDFを作る", systemImage: "doc.fill") .frame(maxWidth: .infinity).padding(8) }.buttonStyle(.borderedProminent).controlSize(.large).disabled(store.isBuilding)
        ...
        if !subscription.isPro { Button("Proで無制限のPDFとテンプレートへ") { showPaywall = true }.frame(maxWidth: .infinity) }
    }.padding(20) }.background(Color(uiColor: .systemGroupedBackground)).navigationTitle("手渡しプリント").sheet(isPresented: $showPaywall) { SubscriptionPaywall(name: "手渡しプリント", benefits: ["無制限のPDF作成", "用途別テンプレート", "再出力履歴"], privacyURL: URL(string: "https://koki-coder-crypto.github.io/lightly-ios/portfolio/privacy.html")!) }.alert("お知らせ", isPresented: Binding(get: { store.error != nil }, set: { if !$0 { store.error = nil } })) { Button("OK", role: .cancel) {} } message: { Text(store.error ?? "") } } }
}
