...
struct PrintHomeView: View {
    ...
    }.padding(20) }.background(Color(uiColor: .systemGroupedBackground)).navigationTitle("手渡しプリント").sheet(isPresented: $showPaywall) { SubscriptionPaywall(name: "手渡しプリント", benefits: ["無制限のPDF作成", "1・2・4写真レイアウト", "端末内でのPDF生成"], privacyURL: URL(string: "https://koki-coder-crypto.github.io/lightly-ios/privacy.html")!) }.alert("お知らせ", isPresented: Binding(get: { store.error != nil }, set: { if !$0 { store.error = nil } })) { Button("OK", role: .cancel) {} } message: { Text(store.error ?? "") } } }
}
