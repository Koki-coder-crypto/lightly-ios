...
struct FocusHomeView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            }.padding(20) }.background(Color(uiColor: .systemGroupedBackground)).navigationTitle("集中ポケット").sheet(isPresented: $showPaywall) { SubscriptionPaywall(name: "集中ポケット", benefits: ["無制限の集中セッション", "集中履歴の保存", "15・25・45分の集中モード"], privacyURL: URL(string: "https://koki-coder-crypto.github.io/lightly-ios/privacy.html")!) }.alert("お知らせ", isPresented: Binding(get: { store.error != nil }, set: { if !$0 { store.error = nil } })) { Button("OK", role: .cancel) {} } message: { Text(store.error ?? "") }
        }
    }
}
