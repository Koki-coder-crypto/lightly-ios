...
private enum AppSpec {
    static let name = "Leave Check"
    static let icon = "checklist"
    static let job = "Keep a short checklist before you leave."
    static let free = "Save up to 3 checklists."
    static let pro = "Unlimited checklists"
    static let action = "Add item"
    static let freeLimit = 3
    static let monthlyID = "jp.egawa.leavecheck.pro.monthly"
    static let yearlyID = "jp.egawa.leavecheck.pro.yearly"
}
...
final class UtilityStore: ObservableObject {
    ...
    struct Entry: Identifiable, Codable { let id: UUID; let title: String; let detail: String; let createdAt: Date; var isDone: Bool }
    private let key = "jp.egawa.leavecheck.entries"
    ...
    func save(isPro: Bool) {
        ...
        entries.insert(Entry(id: UUID(), title: cleaned, detail: detail.trimmingCharacters(in: .whitespacesAndNewlines), createdAt: .now, isDone: false), at: 0)
        title = ""; detail = ""
    ...
    func toggle(_ entry: Entry) { guard let index = entries.firstIndex(where: { $0.id == entry.id }) else { return }; entries[index].isDone.toggle(); if let data = try? JSONEncoder().encode(entries) { UserDefaults.standard.set(data, forKey: key) } }
}
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section("New item") {
                    TextField("Item", text: $store.title)
                    TextField("Note (optional)", text: $store.detail)
                ...
                Section("Checklist") {
                    if store.entries.isEmpty { Text("No items yet").foregroundStyle(.secondary) }
                    ForEach(store.entries) { entry in
                        Button { store.toggle(entry) } label: {
                            HStack(alignment: .top, spacing: 12) {
                                Image(systemName: entry.isDone ? "checkmark.circle.fill" : "circle").foregroundStyle(entry.isDone ? .green : .secondary)
                                VStack(alignment: .leading, spacing: 4) { Text(entry.title).font(.headline).strikethrough(entry.isDone); if !entry.detail.isEmpty { Text(entry.detail).font(.subheadline).foregroundStyle(.secondary) }; Text(entry.createdAt, style: .date).font(.caption).foregroundStyle(.tertiary) }
                            }
                        }.buttonStyle(.plain)
            ...
            .alert("Free limit reached", isPresented: $store.limitReached) { Button("See Pro") { showPaywall = true }; Button("Close", role: .cancel) {} } message: { Text("The free plan stores up to \(AppSpec.freeLimit) checklists.") }
        }
    }
}
