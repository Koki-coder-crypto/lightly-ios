...
private enum AppSpec {
    static let name = "Parcel Note"
    static let icon = "shippingbox.fill"
    static let job = "Keep delivery and pickup notes in one place."
    static let free = "Save up to 5 parcel notes."
    static let pro = "Unlimited parcel notes"
    static let action = "Add parcel"
    static let freeLimit = 5
    static let monthlyID = "jp.egawa.parcelnote.pro.monthly"
    static let yearlyID = "jp.egawa.parcelnote.pro.yearly"
}
...
final class UtilityStore: ObservableObject {
    ...
    private let key = "jp.egawa.parcelnote.entries"
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section("New parcel") {
                    TextField("Delivery or item", text: $store.title)
                    TextField("Pickup detail or note (optional)", text: $store.detail)
                ...
                Section("Saved parcels") {
                    if store.entries.isEmpty { Text("No parcels yet").foregroundStyle(.secondary) }
