...
private enum AppSpec {
    static let name = "Warranty Ledger"
    static let icon = "calendar.badge.clock"
    static let job = "Keep warranty, return, and replacement deadlines in one place."
    static let free = "Save up to 20 records and see the next deadline."
    static let pro = "Unlimited deadline records"
    static let action = "Save deadline"
    static let freeLimit = 20
    static let monthlyID = "jp.egawa.warrantyledger.pro.monthly"
    static let yearlyID = "jp.egawa.warrantyledger.pro.yearly"
}
...
final class UtilityStore: ObservableObject {
    ...
    @Published var deadline = Calendar.current.date(byAdding: .month, value: 1, to: .now) ?? .now
    @Published private(set) var entries: [Entry] = []
    @Published var isRunning = false
    @Published var limitReached = false
    struct Entry: Identifiable, Codable { let id: UUID; let title: String; let detail: String; let deadline: Date; let createdAt: Date }
    private let key = "jp.egawa.warrantyledger.entries"
    ...
    func save(isPro: Bool) {
        ...
        entries.insert(Entry(id: UUID(), title: cleaned, detail: detail.trimmingCharacters(in: .whitespacesAndNewlines), deadline: deadline, createdAt: .now), at: 0)
        entries.sort { $0.deadline < $1.deadline }
        title = ""; detail = ""; deadline = Calendar.current.date(byAdding: .month, value: 1, to: .now) ?? .now
        ...
        NavigationStack {
            List {
                ...
                Section("New deadline") {
                    TextField("Item name", text: $store.title)
                    TextField("Store or note (optional)", text: $store.detail)
                    DatePicker("Deadline", selection: $store.deadline, displayedComponents: .date)
                ...
                Section("Saved deadlines") {
                    if store.entries.isEmpty { Text("No deadlines yet").foregroundStyle(.secondary) }
                    ForEach(store.entries) { entry in
                        VStack(alignment: .leading, spacing: 4) { Text(entry.title).font(.headline); Text(deadlineText(entry.deadline)).foregroundStyle(entry.deadline < .now ? .red : .secondary); if !entry.detail.isEmpty { Text(entry.detail).font(.subheadline).foregroundStyle(.secondary) } }
            ...
            .alert("Free limit reached", isPresented: $store.limitReached) { Button("See Pro") { showPaywall = true }; Button("Close", role: .cancel) {} } message: { Text("The free plan stores up to \(AppSpec.freeLimit) records.") }
        }
    }
    private func deadlineText(_ date: Date) -> String { let days = Calendar.current.dateComponents([.day], from: Calendar.current.startOfDay(for: .now), to: Calendar.current.startOfDay(for: date)).day ?? 0; return days < 0 ? "Expired \(date.formatted(date: .abbreviated, time: .omitted))" : days == 0 ? "Due today" : "Due in \(days) days" }
}
